...
const AddToCartModal = ({ modalData, setModalData, fetchCartData, setCustomSnack, setErrorSnack }) => {
    ...
    const addToCarthandler = async () => {
        ...
        try {
            ...
            if (respData.data.message == "Item Added Successfully") {
                setCustomSnack({ ...setCustomSnack, open: true, message: respData.data.message })
                setModalData({ ...modalData, open: false })
                fetchCartData()
                setBtnLoader(false)
            }
            else {
                setErrorSnack({ ...setErrorSnack, open: true, message: respData.data.message })
                setBtnLoader(false)
            }
        } catch (err) {
            console.log(err);
